...
type UserInfo = { fullName?: string | null; companyName?: string | null; email?: string }
...
export default function DropdownProfile({ align }: { align?: 'left' | 'right' }) {
  ...
  const displayName = user?.companyName || user?.fullName || user?.email || 'Account'
    ...
    <Menu as="div" className="relative inline-flex">
      <MenuButton className="inline-flex justify-center items-center group">
        ...
        <div className="flex items-center truncate">
          <span className="truncate ml-2 text-sm font-medium text-gray-600 dark:text-gray-100 group-hover:text-gray-800 dark:group-hover:text-white">{loading ? 'Loading…' : displayName}</span>
      ...
      >
        <div className="pt-0.5 pb-2 px-3 mb-1 border-b border-gray-200 dark:border-gray-700/60">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 italic">Signed in</div>
        </div>
        <MenuItems as="ul" className="focus:outline-hidden">
          ...
          <MenuItem as="li">
            <button className="w-full text-left font-medium text-sm flex items-center py-1 px-3 text-violet-500" onClick={signOut}>
              Sign Out
            </button>
          </MenuItem>
        </MenuItems>
